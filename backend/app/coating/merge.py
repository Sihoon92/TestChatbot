...
def merge_parquet(inputs, out_path) -> Path:
...
def summarize(merged: pd.DataFrame, out: Path, sources) -> str:
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="기종별 parquet 을 하나로 합친다. 지연(L·τ) 추정 표본을 늘리기 위한 것이다.",
        epilog=(
            "예) python -m app.coating.merge \\\n"
            "      --input data/coating/raw/48X1.parquet data/coating/raw/50S1.parquet \\\n"
            "      --out   data/coating/raw/merged.parquet\n"
            "이후:  python -m app.coating.report --input data/coating/raw/merged.parquet"
        ),
...
def main(argv: list[str] | None = None) -> str:
    ...
    try:
        out = merge_parquet(paths, args.out_path)
    except ValueError as e:
        # 원인은 이미 문장으로 만들어 뒀다. 트레이스백을 그대로 던지면 그 문장이
        # 스택 밑에 묻힌다(report·convert 와 같은 관례).
        raise SystemExit(str(e)) from e

    print(summarize(pd.read_parquet(out), out, [p.name for p in paths]))
    return str(out)
