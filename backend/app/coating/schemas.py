...
COLUMN_ALIASES: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    AT: {
        ...
        # "작업일" 은 날짜만 있는 헤더에서 온다. 시각까지 있는 열과 한 파일에
        # 같이 있으면 충돌로 잡힌다 - 날짜 열과 시각 열을 합치는 것은 추측이라
        # 여기서 할 일이 아니다.
        "ko": ("작업일시", "작업일", "작업시각", "작업시간", "측정시각", "측정시간", "수집시각"),
        "en": ("worktime", "datetime", "timestamp", "measuredat", "collectedat"),
    },
    PRODUCT: {
        "zh-Hans": ("产品", "产品编号", "产品代码", "品名"),
        "zh-Hant": ("產品", "產品編號", "產品代碼"),
        "ko": ("제품", "제품코드", "제품 약칭", "품명", "품번"),
        "en": ("productcode", "productid", "productno"),
    },
    ITEM: {
        "zh-Hans": ("项目编号", "项目代码", "项目号", "点位编号", "参数编号", "测点编号"),
        "zh-Hant": ("項目編號", "項目代碼", "項目號", "點位編號", "參數編號", "測點編號"),
        # MES 에 따라 계측 항목을 "프로젝트" 라고 부른다. 우리가 아는 project 와
        # 뜻이 다르지만, 원본이 그렇게 쓰면 원본을 따른다.
        "ko": ("항목코드", "항목번호", "항목ID", "프로젝트 코드", "태그코드", "포인트코드"),
