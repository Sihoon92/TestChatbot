...
- Produces:
  ...
  - `trace.rule_comparison(iso, event_deltas, bounds, old_pre, old_post, new_pre, new_post) -> dict`
    키: `old`/`new` (각각 `{n_clusters, n_isolated, n_items}`), `both`, `only_new`, `only_old`, `only_old_examples`
...
def _comparison_lines(c) -> list[str]:
    out = [
        "## 3. 구 규칙(연쇄·시작 기준·앞30/뒤60) 대비",
        "",
        "                     구       신",
        f"   묶음          {c['old']['n_clusters']:>6}  {c['new']['n_clusters']:>6}",
        f"   격리 통과     {c['old']['n_isolated']:>6}  {c['new']['n_isolated']:>6}",
        f"   Δgap 항목     {c['old']['n_items']:>6}  {c['new']['n_items']:>6}",
        "",
        f"   양쪽 통과 {c['both']}건 · 신에만 {c['only_new']}건"
        f" · 구에만 {c['only_old']}건   (연속 구간 단위)",
    ]
    if c["only_old_examples"]:
        ...
        for e in c["only_old_examples"]:
            out.append(
                ...
                f" 한 건({e['n_items']}항목) / 신 = 조각 {e['n_fragments']}개"
            )
    return out + [""]
