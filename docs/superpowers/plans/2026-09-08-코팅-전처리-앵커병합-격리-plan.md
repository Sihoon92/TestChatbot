...
- Produces: `trace.timeline(iso, bounds, width=80) -> tuple[float, list[dict]]`
  ...
  - 둘째 값: lot 당 `{lot_id, start, end, cells, runs}`. `cells` 는 `─`(조용)·`✓`(격리 통과)·`✗`(탈락) 세 글자로만 채운 문자열이고, `runs` 는 `[{run_id, n, first_cell, last_cell}]`
  - 한 칸에 두 이벤트가 겹치면 **`✗` 가 이긴다.** 통과한 이벤트 뒤에 탈락이 숨으면 타임라인이 실제보다 깨끗해 보인다
