...
Expected: FAIL — `AttributeError: 'Settings' object has no attribute 'coating_delta_window_minutes'` / `assert 60 == 60` 은 통과하지만 첫 테스트는 `60 != 60`? 실제로는 `isolation_post=60, response_post=60` 이라 첫 테스트가 우연히 통과할 수 있다. 두 번째는 반드시 FAIL 한다.
...
Expected: 전부 PASS. `test_coating_response.py` 나 `test_coating_report.py` 가 `delta_samples` 를 옛 시그니처로 부르면 새 인자로 고친다(`window_minutes=3` → `post_minutes=10, delta_window_minutes=3`).
...
def test_ledger_keeps_run_id_when_a_run_was_split():
    ch = _changes([(0, 0, 40.0, 41.0), (3, 1, 40.0, 41.0), (100, 2, 40.0, 41.0)])
    _, dl, iso = _pipeline(ch)
...
def test_timeline_marks_split_runs():
    ch = _changes([(0, 0, 40.0, 41.0), (3, 1, 40.0, 41.0), (200, 2, 40.0, 41.0)])
    _, _, iso = _pipeline(ch)
    _, rows = trace.timeline(iso, _bounds(), width=40)
    assert rows[0]["runs"]
...
`_isolation_verdict` 는 더 이상 호출되지 않으므로 **삭제**한다(`_gap_verdict` 와 `_merge_verdict` 는 유지).
...
```python
        raise SystemExit(
            "무엇을 진단할지 정해야 한다.\n"
            "  --preprocess <원본 parquet>   전처리 전 과정 (리포트 불필요)\n"
