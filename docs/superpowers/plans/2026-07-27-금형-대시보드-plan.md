...
**Interfaces:**
- Consumes: `useDashboardStore` (Task 6), `types/mold` (Task 4), `formatters` — **아직 없으므로 이 태스크에서는 쓰지 않는다**(Task 8에서 도입 후 Task 11에서 필요 시 정리)
- Produces: `MoldList` 컴포넌트 (props 없음). 행 클릭 시 `navigate(/dashboard/{mold_no})`
...
import { useDashboardStore } from "../../store/dashboardStore";
import { STATUS_LABEL, type MoldSummary } from "../../types/mold";

// 선택 상태는 URL(:moldNo)이 진실이다. 스토어에 따로 두면 링크로 진입했을 때
// 두 값이 어긋난다.
function installationText(mold: MoldSummary): string {
  return mold.line && mold.machine ? `${mold.line}-${mold.machine}` : "—";
}

function defectText(rate: number | null): string {
  // null(미상)을 0으로 렌더하면 "불량 없음"과 구분되지 않는다.
          ...
          <li key={mold.mold_no}>
            ...
            >
              ...
              <span className="block text-xs text-ink/60">
                {STATUS_LABEL[mold.status]} · {installationText(mold)} ·{" "}
                {mold.shot_count.toLocaleString("ko-KR")}타 · 불량 {defectText(mold.latest_defect_rate)}
              </span>
            </button>
          </li>
        );
...
```bash
git add frontend/src/components/Dashboard/MoldList.tsx frontend/src/components/Dashboard/MoldList.test.tsx
git commit -m "feat(dashboard): 금형 목록(좌측 마스터)
...
- Produces:
  ...
  - `SummaryCards.tsx`: `export default function SummaryCards({ detail }: { detail: MoldDetail })`
...
```bash
git add frontend/src/components/Dashboard/formatters.ts frontend/src/components/Dashboard/formatters.test.ts frontend/src/components/Dashboard/SummaryCards.tsx frontend/src/components/Dashboard/SummaryCards.test.tsx
git commit -m "feat(dashboard): 표시 변환기와 요약 카드
...
| `null → —` | Task 8(`formatters`), Task 7·9·10에서 사용 |
...
- Task 8의 `fmtPercent` 시그니처(`(v: number | null, digits = 1)`)가 Task 9의 호출부와 일치한다.
...
- Task 7은 `formatters`(Task 8)보다 먼저 오므로 자체 헬퍼를 쓴다. 순서를 바꾸지 않은 이유는 목록이 상세보다 먼저 동작해야 화면을 눈으로 확인하며 진행할 수 있기 때문이다. Task 11 이후 정리하고 싶다면 별도 리팩터링 커밋으로 한다.
