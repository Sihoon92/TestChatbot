...
- Produces: 아래 심볼 전부. 이후 태스크가 이 이름과 타입에 의존한다.
  ...
  - `MES_FIELDS: list[str]`, `IQC_VALUE_FIELDS: list[str]` — 단계별 고정 필드 어휘. `assemble`(Task 7)과 `discover`(Task 11)가 **둘 다** 쓰므로 여기 둔다. `discover → assemble` 의존은 방향이 어색하다(구조 탐색이 조립 로직을 알 이유가 없다).
...
**Interfaces:**
- Consumes: Task 3 의 `Row`, `MoldRecord`, `StageItemRecord`, `MES_FIELDS`, `IQC_VALUE_FIELDS`; Task 4 의 `normalize_mold_no`, `normalize_status`, `to_int`, `to_float`
- Produces:
...
- Produces:
  - `db.connect(path: str) -> sqlite3.Connection` (row_factory = `sqlite3.Row`)
...
**Interfaces:**
- Consumes: Task 3 의 `SheetLayout`, `SourceKind`, `MES_FIELDS`, `IQC_VALUE_FIELDS`; 기존 `app.excel.tools.make_excel_tools`
- Produces:
